...
def embeddings():
    try:
        ...
        if collection_name in collections:
            # If the collection exists, get the existing collection
            ...
            documents = [
                "Indian Railway Catering and Tourism Corporation (IRCTC) was founded on 27 September 1999.",
                "IRCTC operates under the administrative control of the Ministry of Railways.",
                "IRCTC is a public sector undertaking with 62.4% ownership by the Government of India.",
                "IRCTC was listed on the National Stock Exchange and Bombay Stock Exchange in 2019.",
                "IRCTC's headquarters is located in New Delhi, India.",
                "Sanjay Kumar Jain, IRAS, is the Chairman & Managing Director of IRCTC.",
                "As of 2023, IRCTC has 66 million registered users.",
                "IRCTC books an average of 731,000 tickets daily.",
                "IRCTC's revenue for the fiscal year 2023 was ₹3,541 crore (US$420 million).",
                "IRCTC's operating income for the fiscal year 2023 was ₹1,354 crore (US$160 million).",
                "IRCTC's net income for the fiscal year 2023 was ₹1,006 crore (US$120 million).",
                "IRCTC offers services including online ticketing, catering, tourism, and hospitality.",
                "IRCTC has exclusive rights for onboard catering on all Indian Railways trains.",
                "IRCTC launched the bottled water brand 'Rail Neer' in 2003.",
                "IRCTC operates luxury trains like Maharajas' Express and Deccan Odyssey.",
                "IRCTC operates private trains like the Tejas Express.",
                "IRCTC organizes package tours for domestic and foreign tourists.",
                "IRCTC offers travel insurance through third-party insurers."
            ]
        ...
        logging.error(f"Error in generation of embedding: {e}")



# Call embeddings function immediately after definition
...
def sttt(waveform, sample_rate):
    try:
        ...
        translated_text_from_audio_org = seamless_processor.decode(output_tokens[0].tolist()[0], skip_special_tokens=True)
        translated_text_from_audio = re.sub(r'irccc|ircct|ircc', 'IRCTC', translated_text_from_audio_org, flags=re.IGNORECASE)
...
def llm(translated_text_from_audio, collection):
    try:

        llm_prompt = translated_text_from_audio + " You should always answer the questions under 20 words."
        response = ollama.embeddings(prompt=llm_prompt, model="nomic-embed-text")
        results = collection.query(query_embeddings=[response["embedding"]], n_results=1)
        data = results['documents'][0][0]
        output = ollama.generate(model="gemma:2b", prompt=f"Using this data: {data}. Respond to this prompt: {llm_prompt}")
    ...
    try:
        ...
        if detected_lang in ['tam', 'mal','tel']:
            ...
            with ThreadPoolExecutor() as executor:
                ...
                llm_future = executor.submit(llm, translated_text_from_audio, collection)
